...
export const AdvancedFinancialAnalytics: React.FC<AdvancedFinancialAnalyticsProps> = ({
  ...
  if (error) {
    return (
      <Box variant="default" className={`p-6 border-red-200 bg-red-50 ${className}`}>
    ...
    return (
      <Box variant="default" className={`p-6 ${className}`}>
        <div className="text-center">
          ...
          <h3 className="text-lg font-medium text-gray-900 mb-2">Advanced Analytics Available</h3>
  ...
  if (!hasData && !autoAnalyze) {
    return (
      <Box variant="gradient" className={`p-6 ${className}`}>
        <div className="text-center">
          <div className="w-16 h-16 bg-gradient-to-br from-indigo-500 to-purple-600 rounded-full flex items-center justify-center mx-auto mb-4">
            <ChartBarIcon className="h-8 w-8 text-white" />
          </div>
          <h3 className="text-lg font-medium text-gray-900 mb-2">Advanced Financial Analytics</h3>
          ...
          <button
            ...
            className="inline-flex items-center gap-2 px-6 py-3 bg-gradient-to-r from-indigo-600 to-purple-600 text-white rounded-lg hover:from-indigo-700 hover:to-purple-700 transition-all duration-200 transform hover:scale-105"
          >
    ...
    <Box variant="gradient" className={`p-6 ${className}`}>
      ...
      <div className="mb-6">
        <h3 className="text-xl font-semibold text-gray-800 flex items-center">
          ...
          <span className="ml-3 px-3 py-1 bg-gradient-to-r from-indigo-100 to-purple-100 text-indigo-800 text-sm rounded-full">
            AI-Powered
          </span>
        </h3>
        ...
        <div className="bg-gradient-to-r from-indigo-50 to-purple-50 border border-indigo-200 rounded-lg p-5 mb-6">
          <div className="flex justify-between items-center mb-4">
            <button
              ...
              className="flex items-center gap-2 text-indigo-700 hover:text-indigo-900 transition-colors font-medium"
            >
              ...
              Analytics Settings
              <span className="text-sm text-indigo-600 bg-white px-2 py-1 rounded-full">
                {uniqueCategories.length - excludedCategories.length}/{uniqueCategories.length} included
              </span>
            </button>
            <button
              onClick={() => setShowCategorySettings(!showCategorySettings)}
              className="text-sm bg-indigo-600 hover:bg-indigo-700 text-white px-3 py-1 rounded-full transition-colors"
            >
          ...
          {showCategorySettings && (
            <div className="space-y-4">
                ...
                {uniqueCategories.map((category) => {
                  ...
                  return (
                    <label key={category} className="flex items-center space-x-2 cursor-pointer group">
                      ...
                      />
                      <span className={`text-sm transition-all group-hover:text-indigo-600 ${
              ...
              <div className="flex gap-2 pt-3 border-t border-indigo-200">
                <button
                ...
                <button
                  ...
                  className="px-4 py-2 text-sm bg-white text-indigo-600 border border-indigo-600 rounded-lg hover:bg-indigo-50 transition-colors"
                >
                  Reset to Defaults
                </button>
              </div>
                ...
                <div className="bg-amber-50 border border-amber-200 rounded-lg p-4">
                  <div className="flex items-start justify-between gap-3">
                    <div className="flex items-center gap-2">
                      ...
                      <div>
                        <p className="text-sm font-medium text-amber-800">Settings Updated</p>
                        <p className="text-sm text-amber-700">Changes will take effect after running a new analysis</p>
                      </div>
                    </div>
                    <button
                      onClick={handleManualAnalyze}
                      className="px-4 py-2 text-sm bg-amber-600 hover:bg-amber-700 text-white rounded-lg transition-colors flex-shrink-0"
                    >
                      Re-analyze Now
                    </button>
                  </div>
              ...
              return (
                <div key={index} className="flex justify-between items-center p-3 bg-gray-50 rounded-lg">
                  ...
                  <div className="text-right">
                    <p className="font-semibold text-red-600">{formatCurrency(vendor.total_spent || 0)}</p>
                    <p className="text-sm text-gray-500">{formatCurrency(vendor.average_transaction || 0, 2)} avg</p>
                  </div>
                </div>
              );
            })}
                ...
                {highAnomalies.slice(0, 3).map((anomaly, index) => (
                  <div key={index} className="p-3 bg-red-50 border border-red-200 rounded-lg">
                    <div className="flex justify-between items-start">
                      ...
                      {anomaly.amount && (
                        <p className="font-semibold text-red-700">{formatCurrency(anomaly.amount)}</p>
                      )}
                    </div>
                  </div>
                ))}
                {mediumAnomalies.slice(0, 2).map((anomaly, index) => (
                  <div key={`medium-${index}`} className="p-3 bg-yellow-50 border border-yellow-200 rounded-lg">
                    <div className="flex justify-between items-start">
                      <div>
                      ...
                      {anomaly.amount && (
                        <p className="font-semibold text-yellow-700">{formatCurrency(anomaly.amount, 2)}</p>
                      )}
                    </div>
                  </div>
                ))}
        ...
        <div className="bg-white rounded-lg p-5 border border-gray-200 mb-6">
          <div className="flex justify-between items-center mb-4">
            ...
            <div className="text-xs text-gray-500 bg-gray-100 px-2 py-1 rounded">
              📊 Predicted amounts based on patterns
            </div>
          </div>
          ...
          {result.insights.subscription_analysis.subscriptions && result.insights.subscription_analysis.subscriptions.length > 0 && (
            <div className="space-y-3 mb-4">
              <h5 className="font-medium text-gray-700">Detected Subscriptions ({result.insights.subscription_analysis.subscriptions.length})</h5>
              {result.insights.subscription_analysis.subscriptions.slice(0, 5).map((subscription: any, index: number) => (
                <div key={index} className="p-4 bg-gray-50 border border-gray-200 rounded-lg">
                  <div className="flex justify-between items-start mb-2">
                    <div className="flex-1">
                      <div className="flex items-center gap-2 mb-1">
                        <h6 className="font-medium text-gray-900">{subscription.vendor || 'Unknown Vendor'}</h6>
                    ...
                    <div className="text-right">
                      <p className="font-semibold text-red-600">{formatCurrency(subscription.projected_annual_cost || 0)}/year</p>
                      <p className="text-sm text-gray-500">{formatCurrency(subscription.monthly_estimate || 0)}/month</p>
                      <p className="text-xs text-gray-400">Total: {formatCurrency(subscription.total_spent_so_far || 0)}</p>
                    </div>
                  </div>
                </div>
              ))}
          ...
          {(!result.insights.subscription_analysis.subscriptions || result.insights.subscription_analysis.subscriptions.length === 0) && (
            <div className="text-center py-6 text-gray-500">
              ...
              <p className="text-sm mt-1">Your spending appears to be mostly one-time purchases</p>
      ...
      <div className="mt-6 pt-4 border-t border-gray-200 text-center">
        ...
        <button
          ...
          className="mt-2 inline-flex items-center gap-2 px-4 py-2 bg-gray-600 text-white text-sm rounded-lg hover:bg-gray-700 transition-colors"
        >
          <ArrowPathIcon className="h-4 w-4" />
          Refresh Analysis
        </button>
